module.exports = (options) => {
  ...
  const NODE = Array.from(options.node);
  ...
  px.modifyContainer = (id, options, cb) => {
    _distribution((node) => {
      px.nextId((response) => {
        const params = {};
        if (options.cpu) params.cpuunits = options.cpu;
        if (options.description) params.description = options.description;
        if (options.memory) params.memory = options.memory;
        if (options.disk) params.rootfs = options.disk;
        if (options.swap) params.swap = options.swap;
        if (options.ostype) params.ostype = options.ostype;
        if (options.net) params.net = options.net;
